...
if __name__=="__main__":
            ...
            for parameters in available_parameters:
                ...
                while True: 
                    ...
                    with open(f'quick_results/{args.model}/results_{Nrounds}.csv','a') as table:
                        ...
                        X = timeout(model.find_lowest_weight_xor_differential_trail,(fixed_variables, solver))
    ...
    print('STARTING SECOND PHASE FOR '+str(args.model))  
    for i in range(3):
        end_time = 600*2^(i+1)
        print('current end_time on ' +str(args.model) +' = '+str(end_time))
        for solver in constants.MODEL_LIST[args.model]['solver_list']:
            for creator_file, parameters in failure_queue[solver]: 
                ...
                for Nrounds in range(parameters['number_of_rounds'],7):
                    parameters['number_of_rounds'] = Nrounds
                    cipher = creator(**parameters)
                    fixed_variables = generate_fixed_variables(cipher, test)
                    ...
                    X = timeout(model.find_lowest_weight_xor_differential_trail,(fixed_variables, solver), timeout_duration=end_time)
                                
                    if isinstance(X,str) == False:
                        print(creator_file + ' succeeded on ' +str(Nrounds))
                        if [creator_file,parameters] in failure_queue[solver]:
                            failure_queue[solver].remove([creator_file,parameters])
                    if isinstance(X,str) == True and ([creator_file, parameters] not in failure_queue[solver]):
                        failure_queue[solver].append([creator_file, parameters])
                        print(creator_file + ' failed on '+str(Nrounds))
                        break
                    if isinstance(X,str) == True and ([creator_file, parameters] in failure_queue[solver]):
                        print(creator_file + ' failed again on '+str(Nrounds))
                        break
                    build_time, solve_time, memory, trail_num, weight = handle_solution(X,args.model)    
                    newline = [model.cipher_id, test, model.__class__.__name__, build_time, solve_time, memory, trail_num, weight, solver]
                    print(newline)
